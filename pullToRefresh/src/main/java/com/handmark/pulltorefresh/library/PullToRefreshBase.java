...
//			onPtrRestoreInstanceState(bundle);
	...
	private void pullEvent() {
		...
		if (newScrollValue != 0 && !isRefreshing()) {
            ...
            Log.i("vector - pull",itemDimension + "< "+ newScrollValue);
            Log.i("vector - pull - state ",(mState ==  State.PULL_TO_REFRESH)+ "  " + mState);

			if (mState != State.PULL_TO_REFRESH && itemDimension >= Math.abs(newScrollValue)) {
				setState(State.PULL_TO_REFRESH);
			} else if ((mState == State.PULL_TO_REFRESH ||mState == State.RESET) && itemDimension < Math.abs(newScrollValue)) {
